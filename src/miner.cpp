...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    //note: getblocktemplate never sets nversion for auxpow, this is done in getauxblock only
    //      so the daemon can still mine legacy blocks via getblocktemplate if desired.
